...
*Related Readings: R27 — Estimating Default Probabilities (pending), R29 — Portfolio Credit Risk (pending), [R31 — Credit Derivatives](R31_Credit_Derivatives.md), R38 — Credit Value Adjustment (CVA) (pending)*
...
- **One-Factor Copula Formula — General vs Common-Correlation Form:**
    The PDF presents the **general** form first (per-firm equity correlation aᵢ), then specializes:
    1. **General factor decomposition:** xᵢ = aᵢ·F + √(1−aᵢ²)·Zᵢ
       where aᵢ = correlation of firm i's equity returns with the market index.
    2. **General conditional PD:** Qᵢ(T|F) = N( [N⁻¹(Qᵢ(T)) − aᵢ·F] / √(1−aᵢ²) )
    3. **Under common correlation** (aᵢ = √ρ for all i, i.e., all firms share the same loading on F):
       - xᵢ = √ρ·F + √(1−ρ)·Zᵢ
       - Q(T|F) = N( [N⁻¹(Q(T)) − √ρ·F] / √(1−ρ) )
    Exam pitfall: presenting only the common-correlation form and labeling it as "the" one-factor formula is wrong — the general form with aᵢ is the canonical Schweser presentation.
...
---
**Source Fidelity:** §1 propositions P7 (one-factor copula) re-verified 2026-04-26 against `R30.raw.gemini.md` extracted from FRM 2026 Part II Book 2 PDF, pages 150–162. F2, F3, F4 confirmed match. F1 corrected to show both general form (aᵢ) and common-correlation simplification.
