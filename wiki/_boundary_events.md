...
| CVA / DVA accounting | R30, R38 *(Schema B pending)* | [REG] vs [ECO]: Basel disallows DVA benefit in regulatory capital; accounting standards (IFRS 13) permit it |
...
| CDS spread and hazard rate | R31, R38 *(Schema B pending)* | [THE] vs [OPS]: R31 derives CDS spread from hazard rate under constant λ assumption; R38 extends to CVA incorporating CDS-implied PD as the market-standard input |
...
| Wrong-Way Risk materialises | Counterparty PD rises as bank's exposure rises → CVA increases rapidly → bank's own credit spread widens (DVA rises) → P&L distortion masks true credit deterioration | Credit (CVA/DVA) → Accounting | R30, R38 *(Schema B pending)* |

---

*Add rows to Cross-Domain Links when a new reading's §3 Tensions or §5 Ambiguity Traps reference a concept from another reading. Add rows to Boundary Scenarios when a reading introduces a multi-domain event chain (Step 9 of /new-reading workflow).*
