...
└── wiki/
    ...
    ├── _EVENT_LOG.md                           ← append-only event log (all sessions + system changes)
...
5. **Step 9:** Scan §3 Tensions + §5 Ambiguity Traps → update `wiki/_boundary_events.md` with cross-domain links and boundary scenarios.
...
6. git commit -m "[REVIEW] Week N log"
```
...
4. RECORD    → Cascade updates _LO_TRACKER.md + _ERROR_ARCHETYPES.md + _EVENT_LOG.md
...
```
LO_Readiness  = 0.60 × cumulative_accuracy
